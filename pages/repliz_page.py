...
class ReplizPage(ctk.CTkFrame):
    ...
    def _resource_candidates(self, *parts: str) -> list[Path]:
        rel = Path(*parts)
    ...
    def _first_existing_path(self, *parts: str):
        for p in self._resource_candidates(*parts):
            if p.exists():
                return p
        return None

    def _icons_base_dir(self) -> Path:
        found = self._first_existing_path("assets", "icons")
        if found is not None:
            return found

        if getattr(sys, "frozen", False):
            return Path(sys.executable).resolve().parent / "_internal" / "assets" / "icons"
    ...
    def _build_ui(self):
        ...
        queue_actions.pack(fill="x", padx=22, pady=(0, 10))
        queue_actions.grid_columnconfigure((0, 1, 2), weight=1)
    ...
    def load_accounts(self):
        ...
        for acc in self.accounts:
            ...
            meta_parts = []
            if acc_type:
                meta_parts.append(acc_type)
            if acc_id:
                meta_parts.append(f"ID: {acc_id}")
            ...
            lbl.grid(row=0, column=0, sticky="ew", padx=(10, 6), pady=10)
            ...
            chk.grid(row=0, column=1, sticky="e", padx=(6, 10), pady=10)
    ...
    def get_social_icon(self, account):
        ...
        if "youtube" in hay:
            ...
            icon_name = "linkedin"
        elif "twitter" in hay or provider == "x":
            icon_name = "twitter"
        elif "threads" in hay:
            icon_name = "threads"
        else:
    ...
    def load_video_queue(self):
        ...
        for f in files:
            ...
            ctk.CTkCheckBox(
                ...
                width=24,
                variable=var,
            ).grid(row=0, column=1, sticky="e", padx=(6, 10), pady=10)
